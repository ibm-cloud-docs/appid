...
When you have problems while you are working with {{site.data.keyword.appid_full}}, consider these techniques for troubleshooting and getting help.
{: shortdesc}
...
You can get help by searching for information or by asking questions through a forum. You can also open a support ticket. When you are using the forums to ask a question, tag your question so that it is seen by the {{site.data.keyword.Bluemix_notm}} development teams.
  * If you have technical questions about {{site.data.keyword._short_notm}}, post your question on <a href="http://stackoverflow.com/search?q=ibm+" target="_blank">Stack Overflow <img src="../../icons/launch-glyph.svg" alt="External link icon"></a> and tag your question with "ibm-appid".
...
Here are some possible solutions to get the page to display:
...
## There is no redirect to the app after sign in when using SAML
{: #saml-signin-fail}

{: tsSymptoms}
A user signs into your application through an identity providers sign in page but nothing happens.

{: tsCauses}
An identity provider may no redirect for the following reasons:

* The user is not authorized to use SAML.
* The user tried to sign in with the wrong credentials.

{: tsResolve}

For a redirect to occur, be sure that your user is signing in with the right credentials and that they're configured in your identity provider user settings.
