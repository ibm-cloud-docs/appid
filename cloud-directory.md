...
You can obtain an access token and ID token by supplying the end user's username and the end user's password.
  ```swift
...
Use the LoginWidget class to start the change details flow.
...
Use the LoginWidget class to start the change password flow.
This API can be used only when the user is logged in using Cloud Directory identity provider.
```swift
 class delegate : AuthorizationDelegate {
     public func onAuthorizationSuccess(accessToken: AccessToken?, identityToken: IdentityToken?, response:Response?) {
...
Make sure that the Cloud Directory identity provider set to **ON** in the AppID dashboard,
and include the callback endpoint when using the following.

### Login using resource owner password flow
WebAppStrategy allows users to login to your web application using a username and password.
After a successful login, the user access token will be persisted in the HTTP session, making it available as long as the HTTP session is kept alive. Once the HTTP session is destroyed or expired, the user access token will be destroyed as well.
To allow login using a username and password, add to your app a post route that will be called with the username and password parameters.
```javascript
...
Note:
1. If you submit the request using an HTML form, use [body-parser](https://www.npmjs.com/package/body-parser) middleware.
...
### Sign up
Pass the WebAppStrategy "show" property and set it to `WebAppStrategy.SIGN_UP`. This launches the App ID sign up form.
```javascript
...
### Forgot Password
Pass the WebAppStrategy "show" property and set it to `WebAppStrategy.FORGOT_PASSWORD`. This launches the App ID forgot password form.
```javascript
...
### Change Details
Pass the WebAppStrategy "show" property and set it to `WebAppStrategy.CHANGE_DETAILS`. This launches the App ID forgot password form.
```javascript
...
### Change Password
Pass the WebAppStrategy "show" property and set it to `WebAppStrategy.CHANGE_PASSWORD`. This launches the App ID forgot password form.
```javascript
