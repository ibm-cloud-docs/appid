...
A cloud directory is a user registry that is maintained in the cloud. When a user signs up for your app with their email and a password, they are added to your directory of users. From the user directory you can see how many people have access to your application, who they are, and when they last logged in. You can also search for specific users.

With cloud directory, end users have a little bit more freedom to interact with your application as they can manage their own account. This offers you the chance to have more contact with your users.

![An end users experience of cloud directory](/images/cloud-directory-user.png)
...
You can configure the notifications and level of user control for your app. Setting up your cloud directory can be done quickly in less than 5 steps as shown in the following image.

![Configuring cloud directory](/images/cloud-directory.png)

1. Be sure that cloud directory is turned on as an identity provider.
2. To allow users to use an email to sign up, you must set **Allow users to sign up and reset their password** to **On**. You can still add users through the console when it's set to **Off**, but only for development purposes.
3. Configure your sender details. Specify the email address from which the messages appear to be from, the sender, and to whom your users can reply.
...
5. Using the templates provided, customize your messages with your brand, or personalized messages. For more information see [Managing messages](/docs/services/appid/cloud-directory.html#cd-messages).
6. Review individual users activity in the **Users** tab of the GUI.
