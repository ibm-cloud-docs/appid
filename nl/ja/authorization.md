...
<dl>
  ...
  <dt>リフレッシュ・トークン</dt>
      <dd><p>{{site.data.keyword.appid_short}} は、<a href="http://openid.net/specs/openid-connect-core-1_0.html#RefreshTokens" target="_blank">OIDC <img src="../../icons/launch-glyph.svg" alt="外部リンク・アイコン"></a> で定義されている、再認証なしで新しいアクセス・トークンと識別トークンを取得する機能をサポートしています。
      リフレッシュ・トークンを使用してサインインする場合、ユーザーは資格情報の入力などのアクションを実行する必要はありません。一般に、リフレッシュ・トークンは、通常のアクセス・トークンより存続期間が長くなるように構成されます。</p><p>
      リフレッシュ・トークンを最大限に活用するには、存続期間全体にわたってトークンを維持します。ユーザーは、リフレッシュ・トークンだけでリソースに直接アクセスすることはできません。これにより、アクセス・トークンよりはるかに安全にリソースが維持されるようになります。リフレッシュ・トークンの扱いと、リフレッシュ・トークンを使用して *remember me* 機能を実装する方法の例については、入門用サンプルを確認してください。</p><p>ベスト・プラクティスとして、リフレッシュ・トークンは、それを受け取ったクライアントが安全に保管する必要があり、それを発行した許可サーバーにのみ送られるべきです。</p></dd>
  <dt>許可ヘッダー</dt>
