...
SAML フレームワークは、複数のプロファイル、フロー、および構成をサポートしています。そのため、ID プロバイダー構成を正しく行うことが重要です。SAML を使用する場合によく起きる問題のいくつかについて、解決に役立つトピックを以下に記載します。
{: shortdesc}
...
{{site.data.keyword.appid_short_notm}} は、`RelayState` という非表示のパラメーターを認証要求の一部として送信します。このパラメーターが応答に含まれていない場合は、ID プロバイダーがこのパラメーターを正しく返すように構成されていない可能性があります。 `RelayState` の形式は次のとおりです。
