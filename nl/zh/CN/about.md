...
{{site.data.keyword.appid_short_notm}} 帮助开发者仅需几行代码即可轻松地向 Web 和移动应用程序添加认证，并在 {{site.data.keyword.cloud_notm}} 上保护其云本机应用程序和服务。通过要求用户登录到应用程序，可以存储用户数据（例如，应用程序首选项或来自公共社交个人档案的信息），然后利用数据来定制应用程序中的每个用户的体验。{{site.data.keyword.appid_short_notm}} 为您提供了登录框架，但是使用 Cloud Directory 时，还可以使用自己的品牌屏幕。
{: shortdesc}
...
<dl>
  ...
  <dt>{{site.data.keyword.cloud_notm}}</dt>
    ...
    <strong>Cloud Directory</strong>：用户可以使用其电子邮件和密码来注册服务。然后，您可以通过 UI 管理列表视图中的用户。使用 Cloud Directory 时，{{site.data.keyword.appid_short_notm}} 充当身份提供者。</dd>
  <dt>外部（第三方）</dt>
...
您可以将 {{site.data.keyword.appid_short_notm}} 与其他 {{site.data.keyword.cloud_notm}} 产品配合使用。
{:shortdesc}
...
{{site.data.keyword.appid_short_notm}} 已成功完成多项认证、审计并符合多项标准。
{: shortdesc}
...
{{site.data.keyword.appid_short_notm}} 是在多个专区中运行的高可用性区域服务。
{: shortdesc}

在每个支持的多专区区域中，每个专区都有自己的 {{site.data.keyword.containerlong_notm}} 集群，其中包含多个工作程序节点。每个工作程序节点运行多个 {{site.data.keyword.appid_short_notm}} 组件实例。每个区域的前端为全局负载均衡器和 Web 应用程序防火墙。
