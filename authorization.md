...
<dl>
  ...
  <dt>Claims</dt>
    <dd>OIDC defines a set of standardized claims, or information about a user, that can be returned by an OIDC service. These claims include fields such as <code>name</code>, <code>email</code>, and <code>locale</code> among others; however, OIDC additionally allows for custom claims to be returned. As an OIDC service, App ID collects a subset of OIDC claims that are called normalized claims. When available, they are injected into identity tokens by App ID. All of the other claims can be found from the <code>UserInfo</code> endpoint.</dd>
...
<table>
  ...
  <tr>
    ...
    <td>States that the token is authorized to read the associated App ID profile.</td>
  </tr>
  <tr>
    <td><code>appid_readuserattr</code></td>
    <td>States that the token is authorized to read the associated App ID user's attributes.</td>
  </tr>
  <tr>
    <td><code>appid_writeuserattr</code></td>
    <td>States that the token is authorized to modify the associated App ID user's attributes.</td>
  </tr>
</table>

You can add custom scopes to your tokens by using the App ID custom identity flow.
{: tip}
...
To promote a secure separation or roles for project management and developers, App ID divides its APIs into 2 categories differentiated by the accounts with permission to make the request.

**Runtime**

The [Runtime APIs](https://appid-oauth.ng.bluemix.net/swagger-ui/#/Authorization_Server_V3) contain all of the endpoints necessary for a specific client app to use an App ID instance during app execution. This includes all of the supported endpoints defined by OAuth 2 and OIDC. Each Runtime API is protected with either the clients specific `client id` and `secret` as part of a basic authorization header or an App ID access token.
...
For customizing and managing your App ID instances, you can leverage the App ID [Management APIs](https://appid-management.ng.bluemix.net/swagger-ui/).
...
With the management APIs you can configure identity providers, manage instances of App ID, manage cloud directory users, customize your UI, and more.
